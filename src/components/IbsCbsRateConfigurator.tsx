...
export const IbsCbsRateConfigurator: React.FC<IbsCbsRateConfiguratorProps> = ({
  ...
  const handleSelectPreset = (
    type: 'standard_26_5' | 'transition_2027' | 'test_2026' | 'health_10_6' | 'liberal_18_55' | 'zero_rate' | 'custom'
  ) => {
    ...
    } else if (type === 'liberal_18_55') {
      onChange({
        useCustomIbsCbsRate: true,
        customCbsRatePct: 6.16,
        customIbsRatePct: 12.39,
      });
  ...
  return (
    <div className="bg-white rounded-2xl border border-indigo-100 p-5 sm:p-6 shadow-sm space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-900 tracking-tight flex items-center gap-2">
              Alíquota IBS / CBS Oficial (Lei Complementar nº 214/2025)
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">
                IVA Dual LC 214/2025
              </span>
            </h3>
            <p className="text-xs text-slate-500 font-medium">
              Parâmetros oficiais da Reforma Tributária: CBS Federal (8,80%) e IBS Subnacional (17,70%) ou regimes favorecidos.
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2 bg-indigo-50/70 p-2 rounded-xl border border-indigo-100">
          <span className="text-xs font-bold text-slate-600">Alíquota Efetiva do IVA Dual:</span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
        ...
        >
          <span className="text-[10px] font-black uppercase text-indigo-600 tracking-wider">LC 214 (2027)</span>
          <span className="text-base font-black text-slate-900 mt-1">8,90%</span>
          <span className="text-[11px] text-slate-500 font-medium mt-0.5">CBS 8,8% + IBS 0,1%</span>
        </button>
        ...
        >
          <span className="text-[10px] font-black uppercase text-indigo-600 tracking-wider">Plena (2033)</span>
          <span className="text-base font-black text-slate-900 mt-1">26,50%</span>
          <span className="text-[11px] text-slate-500 font-medium mt-0.5">CBS 8,8% + IBS 17,7%</span>
        </button>
        ...
        >
          <span className="text-[10px] font-black uppercase text-amber-600 tracking-wider">-60% Redução</span>
          <span className="text-base font-black text-slate-900 mt-1">10,60%</span>
          <span className="text-[11px] text-slate-500 font-medium mt-0.5">Saúde, Farma e Agro</span>
        </button>

        <button
          type="button"
          onClick={() => handleSelectPreset('liberal_18_55')}
          className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
            useCustomIbsCbsRate && Math.abs(currentTotal - 18.55) < 0.2
              ? 'border-indigo-600 bg-indigo-50/80 shadow-xs'
              : 'border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50'
          }`}
        >
          <span className="text-[10px] font-black uppercase text-blue-600 tracking-wider">-30% Redução</span>
          <span className="text-base font-black text-slate-900 mt-1">18,55%</span>
          <span className="text-[11px] text-slate-500 font-medium mt-0.5">Profissões Reguladas</span>
        </button>
        ...
        >
          <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Alíquota Zero</span>
        ...
        <button
          ...
          className={`p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
            useCustomIbsCbsRate && Math.abs(currentTotal - 10.6) >= 0.2 && Math.abs(currentTotal - 18.55) >= 0.2 && currentTotal !== 0
              ? 'border-indigo-600 bg-indigo-50/80 shadow-xs'
              : 'border-slate-200 bg-white hover:border-indigo-200 hover:bg-slate-50'
          }`}
        >
          <span className="text-[10px] font-black uppercase text-purple-600 tracking-wider">Personalizada</span>
          <span className="text-base font-black text-slate-900 mt-1">Manual</span>
